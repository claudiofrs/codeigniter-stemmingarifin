...
*
*
*	STEMMING ARIFIN CODEIGNITER LIBRARY 
*	=================================================================
	*	Library ini BUKAN buat dari scratch, 
	*	merupakan konversi dari https://goo.gl/FDJjnn menjadi library
	*	codeigniter untuk memudahkan penggunaan text mining 
	*	pada platform codeigniter
	*
	*	find on web @claudiofrs
	*
*	
*
* 
*/
	...
	*/
	...
	class Stemming_Arifin
	{
		...
		function run($paragraf){
			$arrKalimat = $paragraf;
			$stemmed = array();
			$myfile = fopen(APPPATH.'libraries/StemArifin/dasar', "r") or die("Unable to open file!");
			$readfile = fread($myfile,filesize(APPPATH.'libraries/StemArifin/dasar'));
			fclose($myfile);
			$kamus_arr = explode(" ", $readfile);

			for ($i=0; $i < sizeof($arrKalimat); $i++) { // Kalimat
				for ($j=0; $j < sizeof($arrKalimat[$i]); $j++) { // Kata
					$stemmed[$i][$j] = $this->stemmingArifin($arrKalimat[$i][$j], $kamus_arr);
				}
			}
